...
def query_to_docs_attention(attentions, query_span, doc_spans):
    ...
    all_attn = torch.stack(attentions).squeeze(1)   
    avg_attn = all_attn.mean(dim=(0, 1))           

    q_start, q_end = query_span
    doc_scores = torch.zeros(len(doc_spans), device=attentions[0].device)

    for i, (d_start, d_end) in enumerate(doc_spans):
        sub_matrix = avg_attn[q_start:q_end, d_start:d_end]  
        doc_scores[i] = sub_matrix.sum(dim=-1).mean()
...
def analyze_gold_attention(result, save_path="plot2/gold_attention_plot.png"):
    ...
    position_stats = df.groupby("gold_position")["gold_score"].mean().reset_index()

    plt.figure(figsize=(10, 6))
    plt.plot(position_stats["gold_position"], position_stats["gold_score"], marker="o",linestyle="-",color="b",markersize=4,)

    plt.title("Attention to Gold Tool vs. Position in Prompt")
    plt.xlabel("Position of Gold Tool (0 = Start of Prompt)")
...
def get_query_span(putils, tokenizer, query_text):
    # TODO 3: Query span
    """
    ...
    start_index = len(prefix_tokens)

    query_tokens = tokenizer(query_text, add_special_tokens=False).input_ids
    end_index = start_index + len(query_tokens)
...
if __name__ == '__main__':
    ...
    for qix in tqdm(range(len(test_queries))):
        ...
        query_span = get_query_span(putils, tokenizer, question) 
    ...
    final_ranks = np.array([r["gold_rank"] for r in results])
    print(f"Final Recall@1: {np.mean(final_ranks < 1):.4f}")
    print(f"Final Recall@5: {np.mean(final_ranks < 5):.4f}")
